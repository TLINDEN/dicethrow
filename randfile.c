...
unsigned char *fetch_dict(char *randfile, int min, int max) {
  /*
    read in the random source. we generate an array of max
    66666 entries from the random source, each entry's index will
  ...
  */
  ...
  if((DICT = fopen(randfile, "rb")) == NULL) {
    perror("Could not open dictfile");
    exit(1);
  }

  words = malloc(66666 * sizeof(unsigned char));
 ...
 LOOP:
  while (fread(&in, 1, 1, DICT) == 1) {
    if(in >= min && in <= max) {
      words[pos] = in;

      if(verbose > 1)
	debug("add to wordlist at index %d: %d", pos, in);
